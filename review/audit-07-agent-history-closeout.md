...
本轮复核确认，`tools/validate_links.js` 原本已经覆盖 `编年/`、`纪传/`、`志/`、`论/` 与 `00_体例.md`，但没有覆盖这轮新增大量外链的 `表/` 与 README。

该缺口已在收口时修复。当前链接验证范围包括：
...
`review/` 与 `en/` 暂不纳入中文主书死链验收范围。

同时，`sources/志/index.json` 已补入本轮 Agent 专题的 `topic_indexes` 导航，避免每个专题虽有自己的 source manifest，却无法从总入口发现。

由于当前环境没有执行仓库本地 Node.js 链接扫描与 HTML snapshot 抓取，**这里只记录工具覆盖范围已经修正，不声称整库链接检查已运行通过**。
